...
//     const matchesSearch = [t.title, t.priority, t.status, t.assignedTo, t.linkedTo]
//       .join(" ").toLowerCase().includes(search.toLowerCase());
//     const matchesMyTasks = showMyTasks ? t.assignedTo === "Sujal Das" : true;
//     return matchesSearch && matchesMyTasks;
//   });

//   const openAdd = () => {
//     setEditingTask(null);
//     reset({ title: "", priority: "MEDIUM", status: "PENDING", dueDate: "", assignedTo: "", linkedTo: "" });
//     setIsModalOpen(true);
//   };
...
//     } else {
//       setTasks((prev) => [...prev, { id: Date.now().toString(), ...data, linkedTo: data.linkedTo ?? "" }]);
//     }
...
//           <p className="text-sm text-gray-500 mt-1">All tasks across the team. Use "My Tasks" to filter by logged-in user.</p>
//         </div>
//         <div className="flex gap-2">
//           <button
//             onClick={() => setShowMyTasks(!showMyTasks)}
//             className={`px-4 py-2 rounded-lg text-sm font-medium border transition ${showMyTasks ? "bg-gray-900 text-white border-gray-900" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50"}`}
//           >
//             My Tasks
//           </button>
//           <button
...
//             {filtered.length === 0 ? (
//               <tr>
//                 <td colSpan={7} className="px-6 py-8 text-center text-gray-400">No tasks found.</td>
//               </tr>
//             ) : (
//               filtered.map((task) => (
...
//                   <td className="px-6 py-4 text-gray-600">{task.linkedTo || "—"}</td>
//                   <td className="px-6 py-4 flex gap-2">
//                     <button onClick={() => setStatusModal(task)} className="px-3 py-1 text-xs border border-gray-300 rounded hover:bg-gray-100 transition">Status</button>
//                     <button onClick={() => openEdit(task)} className="px-3 py-1 text-xs border border-gray-300 rounded hover:bg-gray-100 transition">Edit</button>
//                     <button onClick={() => handleDelete(task.id)} className="px-3 py-1 text-xs border border-red-300 text-red-500 rounded hover:bg-red-50 transition">Del</button>
//                   </td>
//                 </tr>
//               ))
//             )}
...
//               <h2 className="text-lg font-semibold text-gray-800">{editingTask ? "Edit Task" : "New Task"}</h2>
//               <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
//             </div>
//             <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
//               <div>
//                 <label className="text-sm text-gray-600 mb-1 block">Title</label>
//                 <input {...register("title")} placeholder="Task title" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
//                 {errors.title && <p className="text-red-500 text-xs mt-1">{errors.title.message}</p>}
//               </div>
//               <div>
//                 <label className="text-sm text-gray-600 mb-1 block">Priority</label>
//                 <select {...register("priority")} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
...
//                 <input {...register("dueDate")} type="date" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
//                 {errors.dueDate && <p className="text-red-500 text-xs mt-1">{errors.dueDate.message}</p>}
//               </div>
//               <div>
//                 <label className="text-sm text-gray-600 mb-1 block">Assigned To</label>
//                 <input {...register("assignedTo")} placeholder="Person name" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
//                 {errors.assignedTo && <p className="text-red-500 text-xs mt-1">{errors.assignedTo.message}</p>}
//               </div>
//               <div>
//                 <label className="text-sm text-gray-600 mb-1 block">Linked To</label>
//                 <input {...register("linkedTo")} placeholder="e.g. Lead: Ankit Sharma (optional)" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
//               </div>
...
//                 <button type="submit" className="px-4 py-2 text-sm bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition">{editingTask ? "Save Changes" : "Add Task"}</button>
//               </div>
//             </form>
//           </div>
//         </div>
...
useEffect(() => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/tasks/${id}`, getAuthHeaders());
      setTasks((prev) => prev.filter((t) => t.id !== id));
    } catch (err) {
